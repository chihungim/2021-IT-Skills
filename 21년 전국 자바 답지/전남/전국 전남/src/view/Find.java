...
public class Find extends BaseFrame {
	...
	JComboBox combo[] = { new JComboBox(loc), new JComboBox() };
		...
		for (int i = 0; i < loc.length; i++) {
			try {
				ResultSet rs = stmt.executeQuery(
						"SELECT distinct SUBSTRING_INDEX(c_name, ' ', -1) FROM cafe where c_address like '%" + loc[i]
								+ "%'");
				while (rs.next()) {
	...
	void search() {
		c.removeAll();
		try {
			ResultSet rs = stmt.executeQuery("SELECT * FROM cafe where c_name like '%" + combo[1].getSelectedItem()
					+ "%' and c_address like '%" + combo[0].getSelectedItem() + "%'");
			while (rs.next()) {
				c.add(new ItemPanel(rs.getString(2), rs.getString(1)));

			}
			for (int i = c.getComponentCount(); i < 12; i++) {
				c.add(sz(new JPanel(), 210, 210));
			}
	...
	class ItemPanel extends JPanel {
		...
		public ItemPanel(String tit, String no) {
			...
			this.add(new JLabel("", 0) {
				...
				public void paint(Graphics g) {
					...
					g.drawOval(20, 0, 175, 175);
					g.setClip(new Ellipse2D.Float(20f, 0, 175, 175));
					g.drawImage(Toolkit.getDefaultToolkit().getImage("./Datafiles/지점/" + tit.split(" ")[0] + ".jpg"),
							20, 0, 180, 180, null);
			...
			s_l.add(btn("지점소개", a -> {
				location = combo[0].getSelectedItem().toString();
				cno = no;
				cname = tit;
				System.out.println(cname);
				new Intro().addWindowListener(new Before(Find.this));
			}));
			s_l.add(btn("예약하기", a -> {
				new Reserve(combo[0].getSelectedItem() + "", tit).addWindowListener(new Before(Find.this));
			}));
