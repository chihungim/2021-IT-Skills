...
public class AddNotice extends BaseFrame {
	...
	JLabel lbl[] = { lbl("아이디", 2), lbl("등록일", 2), lbl("제목", 2), lbl("내용", 2), lbl("공개여부", 2) };
		...
		for (int i = 0; i < btn.length; i++) {
			btn[i].addActionListener(a -> {
				if (a.getActionCommand().equals("등록")) {
					...
					int no = 0;
					try {
						ResultSet rs = stmt.executeQuery("select u_no from user where u_id = '" + uid + "'");
						if (rs.next()) {
							no = rs.getInt(1);
						}
					...
					execute("insert notice values(0, '" + txt[1].getText() + "','" + no + "','" + txt[2].getText()
							+ "','" + area + "','" + 0 + "','" + isOpened + "')");
	...
	void ui() {
		...
		txt[0].setText(uid);
		txt[1].setText(LocalDate.now().toString());
		n_c_w.add(lbl[3]);
		n_c_c.add(area);
		c_w.add(lbl[4]);
		c_c.add(radio[0]);
