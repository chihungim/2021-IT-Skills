...
public class Login extends BaseFrame {
	...
	public Login() {
		...
		add(Util.lbl("Room escape", JLabel.CENTER, 30), "North");
		add(c = new JPanel(new GridLayout(0, 1)));
		add(s = new JPanel(new GridLayout(1, 0, 5, 5)), "South");

		for (int i = 0; i < cap.length; i++) {
			var t = new JPanel();
			t.add(Util.sz(Util.lbl(cap[i], JLabel.LEFT), 60, 15));
			t.add(txt[i]);
			c.add(t);
		}
		...
		for (int i = 0; i < bcap.length; i++) {
			s.add(Util.btn(bcap[i], a->{
				if(a.getActionCommand().equals(bcap[0])) {
					...
					try {
						var rs = DBManager.rs("select u_no, u_name from user where u_id='"+txt[0].getText()+"' and u_pw='"+txt[1].getText()+"'");
						if(rs.next()) {
							uno = rs.getInt(1);
							uname = rs.getString(2);
							Util.iMsg(uname+"님 환영합니다.");
							new Main().addWindowListener(new Before(Login.this));
							txt[0].setText("");
							txt[1].setText("");
						}else {
							Util.eMsg("아이디 또는 비밀번호가 일치하지 않습니다.");
						}
