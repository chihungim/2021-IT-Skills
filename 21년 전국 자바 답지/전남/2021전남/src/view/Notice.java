...
public class Notice extends BaseFrame {
		...
		t.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
				...
				if (e.getClickCount() == 2) {
					try {
						if (bool == false) {
							ResultSet rs = DBManager.rs("select n_content from notice where n_title = '"
					...
					if (t.getValueAt(t.getSelectedRow(), 2).equals(uid)) {
						...
						DBManager.execute("update notice set n_viewcount = n_viewcount+1 where n_title = '"
								+ t.getValueAt(t.getSelectedRow(), 1) + "'");
						repaint();
						revalidate();
					}
	...
	class Accumulate extends JPanel {
		...
		JLabel lbl[] = { Util.lbl("제목 : ", 2, 12), new JLabel(), Util.lbl("작성일", 4, 10), new JLabel(LocalDate.now() + "") };
