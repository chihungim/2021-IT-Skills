...
public class Quiz extends BaseFrame {
	...
	JPanel n_w = new JPanel(new FlowLayout(0)), n_e = new JPanel(new FlowLayout(2)), s_n = new JPanel(),
			s_c = new JPanel();
	JLabel lbl[] = { Util.lbl("기회 : ", 2, 13), Util.lbl("3", 2, 13), Util.lbl("번", 2, 13), Util.lbl(" / ", 2, 13), Util.lbl("변경 횟수 : ", 2, 13),
			Util.lbl("1", 2, 13), Util.lbl("번", 2, 13) };
	...
	void RandomImage() {
		try {
			ResultSet rs1 = DBManager.rs("select t_answer from test");
			while (rs1.next()) {
