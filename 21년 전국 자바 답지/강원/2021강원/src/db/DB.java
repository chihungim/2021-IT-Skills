...
public class DB {
	...
	public DB() {
		...
		try (var ps = con.prepareStatement("update recommend_info set img = ? where title = ? and recommend_no = ?")) {
			try {
				...
				while (rs.next()) {
					ps.setObject(1, new FileInputStream(new File(
							"지급파일/images/recommend/" + map.get(rs.getString(2)) + "/" + rs.getString(3) + ".jpg")));
		...
		execute("create view ll2 as select l2.no, concat(l.name, ' ', l2.name) as name from location2 l2, location l where l2.location_no = l.no");
	}
