					...
					try {
						...
						while (rs1.next()) {
							...
							btn2[k].addActionListener(a2 -> {
								...
								t.setValueAt(aa, t.getSelectedRow(), cnt);
								bool = true;
							});
							k++;
						}
		...
		private void radialChart() {
			...
			chart = new JPanel() {
				...
				protected void paintComponent(Graphics g) {
					super.paintComponent(g); // 페인트의 원래 내용
					...
					for (int i = 0; i < 5; i++) {
						for (int j = 0; j < 6; j++) {
							p[i].addPoint((int) (380 + (100 - i * 20) * Math.cos(j * Math.PI / 3 - (Math.PI / 2))),
									(int) (150 + (100 - i * 20) * Math.sin(j * Math.PI / 3 - (Math.PI / 2))));
						}
