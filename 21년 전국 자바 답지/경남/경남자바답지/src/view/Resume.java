...
public class Resume extends BaseFrame {
	...
	public Resume() {
		...
		combo.addItem("전체");
		try {
			var rs = DBManager.rs("select name from details");
			while (rs.next()) {
	...
	void search() {
		...
		try {
			...
			while (rs.next()) {
				list.add(new ItemPanel(rs.getString(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5), rs.getString(6), rs.getString(7)));
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	class ItemPanel extends JPanel {
		...
		public ItemPanel(String title, String uno, String uname, String email, String level, String dname, String period) {
			this.email = email;
