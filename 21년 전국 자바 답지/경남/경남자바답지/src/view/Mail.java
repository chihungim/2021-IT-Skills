...
public class Mail extends BaseFrame {
	...
	public Mail() {
		...
		this.add(new JLabel("메일함", 0), "North");
		this.add(tab);
		...
		tmp.add(btn[0]=btn("읽음", a->{
			...
			for (int i = 0; i < t.getRowCount(); i++) {
				if (t.getValueAt(i, 0).equals(true)) {
					DBManager.execute("update mail set `read` = 1 where ma_no ='"+t.getValueAt(i, 4).toString()+"'");
				}
			}
			addRow(mailSql("send"));
		}));
		...
		tmp.add(btn[1]=btn("삭제", a->{
			...
			for (int i = 0; i < t.getRowCount(); i++) {
				if (t.getValueAt(i, 0).equals(true)) {
					DBManager.execute("delete from mail where ma_no ='"+t.getValueAt(i, 4).toString()+"'");
				}
			}
			eMsg("삭제가 완료되었습니다.");
			addRow(mailSql("send"));
		...
		chk.addActionListener(a->{
			for (int i = 0; i < t.getRowCount(); i++) {
				t.setValueAt(chk.isSelected(), i, 0);
			}
		});
		...
		t.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (e.getClickCount()==2) {
					DBManager.execute("update mail set `read` = 1 where ma_no ='"+t.getValueAt(t.getSelectedRow(), 4).toString()+"'");
					addRow(mailSql("send"));
	...
	void addRow(String sql) {
		...
		try {
			...
			while (rs.next()) {
				Object row[] = { false, rs.getString(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5), rs.getString(6) };
				if (rs.getString(6).contentEquals("0")) cnt++;
				m.addRow(row);
			}
		...
		if (tab.getSelectedIndex()==2) {
			...
			s.add(btn("보내기", a->{
				...
				if (txt[1].getText().isEmpty()) {
					if (JOptionPane.showConfirmDialog(null, "제목이 지정되지 않았습니다.\n메일을 보내시겠습니까?", "확인", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
						title = "(제목없음)";
					} else {
						txt[1].requestFocus();
						return;
				...
				if (type.contentEquals("user")) {
					sender = DBManager.getOne("select * from manager where email='"+txt[0].getText()+"'");
				} else sender = DBManager.getOne("select * from user where email='"+txt[0].getText()+"'");
				DBManager.execute("insert into mail values(0, '"+division+"','"+no+"','"+sender+"','"+title+"','"+area.getText()+"',curdate(),0)");
