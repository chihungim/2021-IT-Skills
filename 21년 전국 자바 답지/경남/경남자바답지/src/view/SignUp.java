...
public class SignUp extends BaseFrame {
	...
	JCheckBox chk[] = {
		new JCheckBox("<html>필수동의 항목 및 [선택] 개인정보 수집 및 이용동의, [선택] 광고성 정보 <br>이메일/SMS 수신 동의에 일괄 동의합니다."),
		new JCheckBox("[필수] 만 15세 이상입니다."),
		new JCheckBox("[필수] 서비스 이용약관 동의"),
		new JCheckBox("[필수] 개인정보 수집 및 이용 동의"),
		new JCheckBox("[선택] 광고성 정보 이메일/SMS 수신 동의"),
	};
...
//				SELECT distinct SUBSTRING_INDEX(address, ' ', 1) FROM albajava.company where left(SUBSTRING_INDEX(address, ' ', 1), 2)='인천'
				...
				DBManager.execute("insert into user values(0,'"+txt[0].getText()+"','"+txt[1].getText()+"','"+txt[3].getText()+"','"+txt[4].getText()+"@"+mail+"','"+phone+"','"+l.toString()+"','"+gender+" "+txt[8].getText()+"')");
			} catch (Exception e1) {
				eMsg("생년월일을 다시 입력해주세요.");
				return;
			}
