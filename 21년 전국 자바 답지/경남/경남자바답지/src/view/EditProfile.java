...
//				SELECT distinct SUBSTRING_INDEX(address, ' ', 1) FROM albajava.company where left(SUBSTRING_INDEX(address, ' ', 1), 2)='인천'
				...
				DBManager.execute("update set user pw = '"+txt[1].getText()+"', name ='"+txt[3].getText()+"', email = '"+mail+"', phone ='"+phone+"', birth ='"+l.toString()+"', address ='"+combo[2]+" "+txt[8].getText()+"' where u_no ="+uno);
				DBManager.execute("insert into user values(0,'"+txt[0].getText()+"','"+txt[1].getText()+"','"+txt[3].getText()+"','"+txt[4].getText()+"@"+mail+"','"+phone+"','"+l.toString()+"','"+combo[2]+" "+txt[8].getText()+"')");
			} catch (Exception e1) {
				eMsg("생년월일을 다시 입력해주세요.");
				return;
			}
