...
public class WriteResume extends BaseFrame {
	...
	int point[][] = { {16,13,150,180},{216,36,250,136},{16,200,546,93},{139,350,80,30},{18,380,543,130},{53,520,152,32},{449,560,112,40} };
	...
	JCheckBox chk[] = { new JCheckBox("아르바이트"), new JCheckBox("계약직"), new JCheckBox("파견직"), new JCheckBox("인턴직"), new JCheckBox("정규직") };
	String path;
...
//		this.add(new MyPanel());
		...
		try {
			var rs = DBManager.rs("SELECT name, concat(gender, '자'), concat(SUBSTRING_INDEX(birth, '-', 1), '년생'), SUBSTRING_INDEX(address, ' ', 1), phone, email FROM user where u_no="+uno);
			if (rs.next()) {
		...
		p[6].add(btn("작성완료", a->{
			if (!(txt[0].getText().isEmpty() || txt[1].getText().isEmpty() || txt[2].getText().isEmpty() || txt[3].getText().isEmpty())) {
				eMsg("빈칸이 있습니다.");
				return;
			}
			if (!(combo[0].getSelectedIndex()==-1 || combo[1].getSelectedIndex()==-1 || combo[2].getSelectedIndex()==-1)) {
				eMsg("빈칸이 있습니다.");
				return;
			}
			if (!(chk[0].isSelected() || chk[1].isSelected() || chk[2].isSelected() || chk[3].isSelected() || chk[4].isSelected())) {
				eMsg("빈칸이 있습니다.");
				return;
			}
			...
			String dno = DBManager.getOne("select name from details where name ='"+txt[1].getText()+"'");
			String eno="";
			for (int i = 0; i < chk.length; i++) {
				if (chk[i].isSelected()) {
					eno = i+1+"";
			...
			DBManager.execute("insert into information values(0, '"+uno+"','"+txt[0].getText()+"','"+combo[0].getSelectedItem()+"','"+dno+"','"+eno+"','"+period+"','"+anonymous+"')");
