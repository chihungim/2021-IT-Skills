	...
	String mailSql(String division) {
		...
		if (type.contentEquals("user")) { // 현재 로그인 타입 user
			if (division.contentEquals("send")) { // 받는메일
				sql = "select ma.title, u.email, ma.date, ma.ma_no, ma.detail, ma.read from mail ma, user u where division=2 and sender="+uno+" and ma.recipient=u.u_no";
			} else {
				sql = "select ma.title, u.email, ma.date, ma.ma_no, ma.detail, ma.read from mail ma, user u where division=1 and recipient="+uno+" and ma.recipient=u.u_no";
			}
		} else {
			if (division.contentEquals("send")) { // 받는메일
				sql = "select ma.title, m.email, ma.date, ma.ma_no, ma.detail, ma.read from mail ma, manager m where division=1 and sender="+mno+" and ma.recipient=m.m_no";
			} else {
				sql = "select ma.title, m.email, ma.date, ma.ma_no, ma.detail, ma.read from mail ma, manager m where division=2 and recipient="+mno+" and ma.recipient=m.m_no";
			}
		}
		return sql;
	}
...
//		return new ImageIcon(img.getScaledInstance(img.get, height, hints));
//	}
	...
	void addItem(JComboBox combo, String sql) {
		try {
