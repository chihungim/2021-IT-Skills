...
public class BrandAlba extends BaseFrame {
	...
	public BrandAlba() {
		...
		try {
			var rs= DBManager.rs("select c_no, name from company where c_no <= 25 order by name");
			while (rs.next()) {
				int no = rs.getInt(1);
				...
				jp.addMouseListener(new MouseAdapter() {
					...
					public void mouseClicked(MouseEvent e) {
						if (DBManager.getOne("select c.c_no, c.name from company c, recruitment r where c.c_no=r.c_no and c.c_no <= 25 and c.c_no="+no+" and r.deadline >= curdate() order by name").isEmpty()) {
							eMsg("채용정보가 없습니다.");
							return;
						}
