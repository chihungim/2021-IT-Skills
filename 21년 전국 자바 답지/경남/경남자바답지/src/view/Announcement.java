...
public class Announcement extends BaseFrame {
	...
	public Announcement() {
		...
		s.add(sz(btn("등록", a -> {
			...
			String date = "curdate()";
			String d_no = DBManager.getOne("select * from details where name ='" + txt[3].getText() + "'");
			String e_no = (combo[9].getSelectedIndex() + 1) + "";
			String benefit = "";
			for (int i = 0; i < benefits.size(); i++) {
				benefit += DBManager.getOne("select * from benefit where name ='" + benefits.get(i) + "'") + ",";
			}
			benefit = benefit.substring(0, benefit.length() - 1);

			iMsg("공고가 등록되었습니다.");
			DBManager.execute("insert into recruitment values(0, '" + mno + "','" + title + "','" + period + "','"
					+ week + "','" + time + "','" + standard + "','" + salary + "',curdate(),'" + d_no + "','" + e_no
					+ "','" + benefit + "','" + deadline + "',0)");
			this.dispose();
