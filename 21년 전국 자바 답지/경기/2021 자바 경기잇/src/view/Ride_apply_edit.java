...
public class Ride_apply_edit extends BaseFrame {
	...
	void btn_event() {
		...
		if (!BaseFrame.getOne("select * from ride where r_name ='" + txt[0].getText() + "' and r_no <> " + r_no)
				.isEmpty()) {
			eMsg("놀이기구명이 중복되었습니다.");
			return;
		}
		...
		if (mode == EDIT) {
			...
			if (txt[0].getText().equals(list.get(0)) && txt[1].getText().equals(list.get(1))
					&& txt[2].getText().equals(list.get(2)) && txt[3].getText().equals(list.get(3))
					&& box[0].getSelectedItem().toString().equals(list.get(4))
					&& box[1].getSelectedItem().toString().equals(list.get(5)) && area.getText().equals(list.get(6))
					&& icon.equals(lbl_img.getIcon())) {
				eMsg("수정사항이 없습니다.");
				return;
			}
