...
public class MoreInfo extends BaseFrame {
	...
	void ui() {
		...
		w.add(setBorder(imglbl, new LineBorder(Color.BLACK)));
		...
		setBorder(area, new LineBorder(Color.BLACK));
		w.setOpaque(false);
		c.setOpaque(false);
		s.setOpaque(false);
		c_c.setOpaque(false);
		...
		setBorder(c, new EmptyBorder(0, 20, 0, 20));
	...
	void btn_event() {
		...
		if (val(getOne("select count(*) from ticket where t_date = curdate() and r_no=" + no)) > rei(info[0])) {
