...
public class LocationInfo extends BaseFrame {
	...
	void ui() {
		...
		sz(e, 230, 1);
		setBorder(e, new EmptyBorder(0, 10, 0, 10));
	}
	...
	void events() {
		...
		tab.addChangeListener(a -> {
			...
			ch_img.setIcon(img("./datafile/캐릭터/" + character[r.nextInt(2)] + (r.nextInt(3) + 1) + ".jpg", 200, 200));
		});
		...
		combo.addItemListener(i -> {
			...
			prevIcon = iconlbl[Y][X];

			timer_blink = new Timer(1000, a -> {
