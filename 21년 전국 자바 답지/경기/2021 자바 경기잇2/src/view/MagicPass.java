...
public class MagicPass extends BaseFrame {
	...
	void ui() {
		...
		for (int i = boom; i < 16; i++) {
			...
			JLabel lbl;
			lbls.add(lbl = new JLabel(
					img("./datafiles/이미지/" + getOne("select r_name from ride where r_no =" + n) + ".jpg", 105, 105)));
			lbl.setName(n + "");
		}

		Collections.shuffle(lbls);
		for (var l : lbls) {
			setBorder(l, new LineBorder(Color.BLACK));
			c.add(l);
		}
	...
	void btn_event() {
		...
		if (no.contentEquals("0")) {
			...
			Main.lbl[4].setText("매직패스(" + (val(Main.lbl[4].getText()) - 1) + ")");
			Main.enbLabel(Main.USER);
			dispose();
			return;
		} else {
			if (rei(getOne("SELECT count(*) FROM ticket where t_date = curdate() and r_no=" + no)) >= rei(
	...
	void data() {
		cnt = rei(getOne("select count(*) from ride"));
	}
