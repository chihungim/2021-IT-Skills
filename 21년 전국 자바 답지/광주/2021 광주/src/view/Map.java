...
public class Map extends BaseDialog {
	...
	public Map(BaseDialog b, Point uPos, Point rPos, Point sPos, int rno) {
		this(b);
		this.uPos = uPos;
		this.rPos = rPos;
		this.sPos = sPos;
					...
					maps[i][j].addMouseListener(new MouseAdapter() {
						...
						public void mouseClicked(MouseEvent e) {
							if (((Structure) e.getSource()).type == 1) {
								BaseDialog.eMsg("길은 선택할 수 없습니다.");
								return;
							}

							if (((Structure) e.getSource()).type != 0) {
								BaseDialog.eMsg("이미 사용중인 위치입니다.");
								return;
							}
