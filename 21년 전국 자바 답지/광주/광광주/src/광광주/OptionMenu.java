...
public class OptionMenu extends BaseDialog {
	...
	public OptionMenu(Restaurant r, Restaurant.Menu.ItemRow.Item item) {
		super(r, "주문표에 추가", 300, 700);
		this.price = item.price;
		this.mno = item.no;
		add(BaseFrame.size(img = new JLabel(new ImageIcon(Toolkit.getDefaultToolkit()
				.getImage("./지급자료/메뉴/" + item.no + ".png").getScaledInstance(300, 350, Image.SCALE_SMOOTH))), 300, 350),
				"North");
		add(s = new JPanel(new FlowLayout()), "South");

		System.out.println(BaseFrame.orderList.size());

		add(pane = new JScrollPane(c = new JPanel(new BorderLayout())));
		c.add(BaseFrame.lbl("<html><font face = \"맑은 고딕\"; size = \"6\";><b>" + item.name
				+ "</b><br></font><font face = \"맑은 고딕\"; size = \"4\";>" + item.dsp, JLabel.LEFT), "North");
		...
		try {
			...
			if (!rs.next()) {
				JLabel lbl;
				c_c.add(BaseFrame.size(lbl = BaseFrame.lbl("옵션이 없는 상품입니다.", 0, 15), 250, 200));
		...
		s.add(add = btn(count.getText().replaceAll(".*[^0-9^].*", "") + "개를 주문표에 추가하기", a -> {
			...
			int idx = BaseFrame.orderList.size() + 1;

			BaseFrame.orderList.add(list);
			r.orderCount.setText("주문표(" + (BaseFrame.toInt(r.orderCount.getText().replaceAll("[^0-9]", "")) + 1) + ")");
			dispose();
		}));
