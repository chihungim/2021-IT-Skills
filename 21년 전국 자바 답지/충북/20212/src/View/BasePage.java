...
public class BasePage extends JPanel {
	...
	static void curPlaying(int song, int length, String songName) {
		...
		mf.album_img.setIcon(
				new ImageIcon(Toolkit.getDefaultToolkit().getImage(MainFrame.IMG_PATH + "album/" + album + ".jpg")
...
//		mf.album_img.setIcon(new ImageIcon(Toolkit.getDefaultToolkit().getImage(MainFrame.IMG_PATH + "album/" +  + ".png")
//						.getScaledInstance(100, 100, Image.SCALE_SMOOTH)));
	}

	static void setCur(Object row[]) {
		cur_m.setRowCount(0);
		if (row == null || cache.size() == 0) {
		...
		if (!row[2].equals("재생중이지 않음")) {
			curPlaying(toInt(row[5]),
					LocalTime.of(0, toInt(row[4].toString().split(":")[0]), toInt(row[4].toString().split(":")[1]))
							.toSecondOfDay(),
					row[2].toString());
		}
		...
		try {
			...
			if (rs1.next()) {
				cur_m.addRow(new Object[] { rs1.getInt(1) == 0 ? "" : "★", "", rs1.getString(2),
						rs1.getInt(3) == 0 ? "♡" : "♥", rs1.getString(4), rs1.getString(5) });
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	static void prev() {
		...
		var song = cache.pop();
		setCur(cur_m.getDataVector().get(0).toArray());
		que.add(0, song);
		reFresh();
	}
