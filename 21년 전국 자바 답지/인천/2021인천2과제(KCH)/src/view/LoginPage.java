...
public class LoginPage extends BasePage {
	...
	public LoginPage() {
		...
		for (int i = 0; i < bcap.length; i++) {
			int idx = i;
			grid.add(Tools.size(Tools.btn(bcap[i] + "로 로그인", a -> {
				login = idx;
				this.loginField();
			}), 150, 150));
		}
	...
	void loginField() {
		...
		for (int i = 0; i < cap.length; i++) {
			...
			tmp.add(Tools.size(txt[i], 230, 35));
			c_c.add(tmp);
			tmp.setOpaque(false);
		}
		...
		s_grid.add(Tools.size(Tools.btn(bcap[login] + "로 로그인", a -> {
