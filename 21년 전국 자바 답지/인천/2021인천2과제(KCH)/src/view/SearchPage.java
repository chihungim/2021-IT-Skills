...
public class SearchPage extends BasePage {
	...
	public SearchPage() {
		...
		n_f.add(Tools.size(Tools.btn("<html>&#x1F50D", a -> {
			count = 0;
		...
		for (int i = 0; i < cap.length; i++) {
			var tmp = new JPanel();
			tmp.add(Tools.size(new JLabel(cap[i]), 60, 25));
			if (i == 0) {
				tmp.add(sellCombo);
			} else if (i == 1) {
				tmp.add(cateCombo);
		...
		scr_p.add(Tools.lbl("장바구니", 0, 20), "North");
		scr_p.add(scr2 = new JScrollPane(orderPanel = new JPanel(new FlowLayout(0))));
		scr_p.add(Tools.size(Tools.btn("구매하기", a -> {
			if (idSet.size() == 0) {
				Tools.eMsg("장바구니가 비어있습니다.");
				return;
			}
			mf.addPage(new PurchasePage((sellCombo.getSelectedIndex() + 1), orderPanel));

		}), 1, 25), "South");
		Tools.size(scr2, 230, 25);
		Tools.setEmpty(scr_p, 0, 10, 0, 0);
		...
		Tools.setEmpty(this, 10, 10, 10, 10);
	...
	private void search() {
		...
		if (cateCombo.getSelectedIndex() != 0) {
			sql += " and c.c_No = " + Tools.toInt(
					Tools.getOneRef("select c_No from category where c_Name ='" + cateCombo.getSelectedItem() + "'"));
		}
		...
		try {
			...
			while (rs.next()) {
				...
				b_w.add(lbl = Tools.size(new JLabel(Tools.img("./datafile/image/" + name + ".jpg", 80, 80)), 80, 80));
				Tools.setLine(lbl);
				...
				b_s.add(btn = Tools.size(Tools.btn("장바구니에 추가", a -> {
					if (idSet.contains(no)) {
						Tools.eMsg("이미 추가된 상품입니다.");
						return;
					}
	...
	class OrderItem extends JPanel {
		...
		public OrderItem(int no, String name, int price, int stock, int state) {
			...
			cp.add(Tools.size(Tools.lbl(txt, JLabel.LEFT), 150, 25));
			add(lbl=Tools.size(new JLabel(Tools.img("./datafile/image/"+name+".jpg", 30, 30)), 30, 2), "West");
			add(Tools.size(Tools.btn("x", a->{
