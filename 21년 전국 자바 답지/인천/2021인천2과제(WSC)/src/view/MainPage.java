	...
	void popInit() {
		...
		try {
			...
			while (rs.next()) {
				...
				bord.add(img = Util.sz(new JLabel(Util.img("./datafile/image/" + rs.getString(2) + ".jpg", 80, 100)),
						80, 100));
				Util.setLine(img);
	...
	void eventInit() {
		...
		try {
			...
			while (rs.next()) {
				JPanel bord = new JPanel(new BorderLayout());
				JLabel img;
				bord.add(img = Util.sz(new JLabel(Util.img("./datafile/image/" + rs.getString(1) + ".jpg", 80, 80)), 80,
						80));
				Util.setLine(img);
