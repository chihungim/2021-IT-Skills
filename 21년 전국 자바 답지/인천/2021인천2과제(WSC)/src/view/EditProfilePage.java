...
public class EditProfilePage extends BasePage {
	...
	public EditProfilePage() {
		...
		c_s.add(Util.sz(Util.btn("수정", a->{
			...
			Tools.execute("update user set u_Pattern = '"+txt[1].getText()+"', u_Name ='"+txt[2].getText()+"', u_Addr = '"+getKey(combo.getSelectedItem().toString())+"' where u_No="+uno);
	...
	void addItem() {
		try {
			var rs = Tools.rs("SELECT po.* FROM point po left join user u on u.u_Addr=po.po_No left join seller s on s.s_Addr=po.po_No where u.u_No is null and s.s_No is null or u_No="+uno+" order by po_X, po_Y");
			while (rs.next()) {
		...
		combo.setSelectedItem(hashPoint.get(Tools.getOneRs("SELECT u_Addr from user where u_No="+uno)));
	}
