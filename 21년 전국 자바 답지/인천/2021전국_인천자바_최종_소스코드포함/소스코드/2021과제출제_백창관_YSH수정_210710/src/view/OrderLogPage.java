...
public class OrderLogPage extends BasePage {
	...
	public OrderLogPage() {
		...
		scr_p.add(Util.lbl(uname+"님의 구매목록", 0, 25), "North");
		...
		item.addActionListener(a->{
			...
			if (yesno == JOptionPane.YES_OPTION) {
				Util.iMsg("취소되었습니다.");
				// purchase row 삭제
				DBManager.execute("delete from purchase where pu_No='"+selc_puno+"'");
				repaint();
				revalidate();
			}
		});
...
//				9 8 4 10 11 13 12 6 3
				c.add(new ProductItem(rs.getString(9), rs.getString(8), rs.getString(4), rs.getInt(10), rs.getInt(11),
						rs.getInt(13), rs.getString(12), rs.getInt(6), rs.getInt(3), LocalDateTime.parse(rs.getString(14), DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")), rs.getInt(3), rs.getInt(1)));
			}
...
//			s.add(new JLabel("<html><font color =\"gray\">"+date), "West");
...
//			}
//			
			// 배달이 시작되지 않았다면 '배달 준비', 배달이 완료되었다면 '배달 완료' 로 표시하고, 버튼 비활성화
			
			JButton delivery;
			s.add(delivery=Util.btn(state, e->{
				sname = seller;
				usermodeStartTime = a;
				s_addr = saddr;
