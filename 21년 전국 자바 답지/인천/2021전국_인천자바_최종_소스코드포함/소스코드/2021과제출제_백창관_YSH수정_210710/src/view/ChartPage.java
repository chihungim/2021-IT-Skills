...
public class ChartPage extends BasePage {
	...
	void formInit() {
		max = Util.rei(DBManager.getOneRs("select max(sub.s) from (select c_No, pu_Date, sum(pu.pu_Count) as s from purchase pu, product p where pu.p_No=p.p_No and pu.s_No="+BasePage.sno+" group by pu_Date, p.c_No) as sub;"));
