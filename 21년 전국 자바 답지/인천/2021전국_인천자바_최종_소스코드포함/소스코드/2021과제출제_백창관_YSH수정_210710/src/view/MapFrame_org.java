...
//	public MapFrame(String kind, LocalDateTime a) {
...
//		this.a = a;
		...
		if(kind.contentEquals("seller")) {
			...
			for(var i: sellerDeliveryMap.keySet())
			{
				...
				// 수정
				shortestPath(userAddr, showidx++, LocalDateTime.parse((String)sellerDeliveryMap.get(i)[1], DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")));
			}
			hasMinPath = false;
	...
	private void showDeliveryState(int showidx) {
		...
		lbl.setText("<html><br> ○ 출발지점 " + stag + jrb.get(cpath.get(0)-1).getText() + etag +
				    "<br><br> ○ 도착지점 " + stag + jrb.get(cpath.get(eidx)-1).getText() + etag +
				    "<br><br> ○ 최단거리 " + stag + pathDim[1][cpath.get(eidx)] + "m" + etag +
				    "<br><br> ○ 소요시간 " + stag + String.format("%02d", totalSec/3600) + ":" + String.format("%02d", (totalSec%3600)/60) + ":" + String.format("%02d", totalSec%60) + etag + 
				    "<br><br> ○ 배송시간 " + stag + DateTimeFormatter.ofPattern("HH:mm:ss").format(ctime.get(0)).toString() + etag + 
				    "<br><br> ○ 도착시간 " + stag + DateTimeFormatter.ofPattern("HH:mm:ss").format(ctime.get(eidx)).toString() + etag +
				    "</html>");
		jp.add(lbl);
	...
	private void shortestPath(int pathidx, int showidx, LocalDateTime startTime) {
		...
		if(showidx == 0) {
			...
			jp.add(lb1);
			lb1.setBounds(jrb.get(cpath.get(0)-1).getLocation().x, jrb.get(cpath.get(0)-1).getLocation().y-20, 60, 25);
		}
		
		int eidx = cpath.size()-1;
		JLabel lb2 = new JLabel("도착", 0);
		jp.add(lb2);
		lb2.setBounds(jrb.get(cpath.get(eidx)-1).getLocation().x, jrb.get(cpath.get(eidx)-1).getLocation().y-18, 60, 25);
		...
		for (int i = 0; i < cpath.size(); i++) {
			...
			if (kind.contentEquals("seller") && i < cpath.size()-1)
				...
				jrb.get(cpath.get(i) - 1).setToolTipText(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss").format(ctime.get(i)).toString());
		}
	}
	...
	void dataInit() {
		//locMap 데이터 준비
		try {
			var rs = DBManager.rs("select * from point");
			while (rs.next()) {
		...
		try {
			var rs = DBManager.rs("SELECT * from user");
			while (rs.next()) {
		...
		try {
			var rs = DBManager.rs("SELECT p.u_No, p.pu_No, r.r_Time FROM purchase as p, receive as r where p.pu_No = r.pu_No and p.s_No = " + BasePage.sno + " and date(r.r_Time) = date(now())");
			while (rs.next()) {
//				sellerDeliveryMap.put(rs.getInt(1), new Object[] {rs.getInt(2), rs.getDate(3)});
		...
		try {
			var rs = DBManager.rs("select * from connect");
			while (rs.next()) {
