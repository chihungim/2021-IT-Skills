...
//			}
		...
		try {
			var rs = DBManager.rs("select c_Name from category");
			while (rs.next()) {
		...
		try {
			var rs = DBManager.rs("select s_Name from seller");
			while (rs.next()) {
	...
	void search() {
		...
		if (cateCombo.getSelectedIndex() != 0) 
			sql += " and c.c_No = "+Util.rei(DBManager.getOneRs("select c_No from category where c_Name ='"+cateCombo.getSelectedItem()+"'"));;
