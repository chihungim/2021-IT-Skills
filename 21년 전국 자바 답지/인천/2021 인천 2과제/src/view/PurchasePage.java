...
public class PurchasePage extends BasePage {
	...
	public PurchasePage(int selcNo, JPanel p) {
		...
		for (var comp : p.getComponents()) {
			...
			Object row[] = { new JLabel(getImage("./datafile/image/" + item.name + ".jpg", 80, 80)), item.name,
		...
		t.getModel().addTableModelListener(e -> {
			if (e.getType() == TableModelEvent.UPDATE) {
				...
				if (c == 5) {
					if (BasePage.toInt(t.getValueAt(r, c)) > BasePage.toInt(t.getValueAt(r, c - 1))) {
						BasePage.eMsg("재고 개수를 초과하여 구매할 수 없습니다.");
						t.setValueAt(BasePage.toInt(t.getValueAt(r, c - 1)), r, c);
					}
					double st = (t.getValueAt(r, c - 3).toString().contentEquals("O")) ? 0.5 : 1;
					int price = (int) (BasePage.toInt(t.getValueAt(r, c - 2)) * BasePage.toInt(t.getValueAt(r, c))
							* st);
					t.setValueAt(price, r, c + 1);
					int sum = 0;
					for (int i = 0; i < t.getRowCount(); i++) {
						sum += BasePage.toInt(t.getValueAt(i, c + 1));
					}
			...
			for (int i = 0; i < t.getRowCount(); i++) {
				sum += BasePage.toInt(t.getValueAt(i, 6));
			}
