...
public class DB extends JFrame {
	...
	public DB() {
		...
		try {
			...
			execute("user 권한 설정", "Grant select, insert, delete, update on Delivery.* to user@localhost");
			...
			createT("seller",
					"s_No int primary key not null auto_increment, s_Id varchar(20) not null, s_Pattern varchar(10) not null, s_Name varchar(20) not null, s_Addr int not null, foreign key(s_Addr) references point(po_No) "
							+ cascade);
			createT("category", "c_No int primary key not null auto_increment, c_Name varchar(20) not null");
			...
			createT("event",
					"e_No int primary key not null auto_increment, p_No int, e_Month int, foreign key(p_No) references product(p_No) "
							+ cascade);
			createT("purchase",
					"pu_No int primary key not null auto_increment, s_No int, p_No int, u_No int, pu_Count int, pu_Price int, pu_Date date, foreign key(s_No) references seller(s_No) "
							+ cascade + ", foreign key(p_No) references product(p_No) " + cascade
	...
	void iLog(String t, boolean b) {
		JLabel clbl = new JLabel((b ? "성공" : "실패"), b ? iIcon : eIcon, 0);
