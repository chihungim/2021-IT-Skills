...
public class ChartPage extends BasePage {
	...
	void formInit() {
		max = Tools.toInt(Tools.getOneRef("select max(sub.s) from (select c_No, pu_Date, sum(pu.pu_Count) as s from purchase pu, product p where pu.p_No=p.p_No and pu.s_No="+BasePage.sno+" group by pu_Date, p.c_No) as sub;"));
		...
		n.add(prev = Tools.btn("◀", a -> {
			this.setMonth(-1);
		}));
		n.add(lblDate = Tools.lbl(format.format(date), 0, 20));
		n.add(next = Tools.btn("▶", a -> {
			this.setMonth(1);
		}));

		Tools.size(prev, 35, 35);
		Tools.size(next, 35, 35);
	...
	void setCal() {
		...
		for (int i = 0; i < days.length; i++) {
			...
			try {
				var rs = Tools.rs(
						"select c.c_No, c.c_Name, sum(pu.pu_Count) from product p inner join category c on c.c_No=p.c_No inner join purchase pu on pu.p_No=p.p_No where pu.s_No="+BasePage.sno+" and pu.pu_Date='"
								+ l.toString() + "' group by c.c_No order by c.c_No");
				...
				days[i].addMouseListener(new MouseAdapter() {
					...
					public void mouseClicked(MouseEvent e) {
						...
						dialog.add(Tools.lbl(l.toString()+" 매출 실적", 0, 15), "North");
						...
						dialog.add(p);
						dialog.add(s, "South");
						s.add(Tools.size(Tools.btn("OK", a->{
							dialog.dispose();
							setMonth(0);
							repaint();
							revalidate();
						...
						Tools.size(type, 100, 100);
						
						dialog.getContentPane().setBackground(Color.white);
						dialog.setModal(true);
						dialog.setUndecorated(true);
						((JPanel)dialog.getContentPane()).setBorder(new CompoundBorder(new LineBorder(Color.black, 2), new EmptyBorder(10, 10, 10, 10)));
	...
	class ChartPanel extends JPanel {
		...
		public ChartPanel(String day, Color foreColor) {
			...
			var lbl = Tools.lbl(day, JLabel.LEFT, 15);
			this.add(lbl, "North");
