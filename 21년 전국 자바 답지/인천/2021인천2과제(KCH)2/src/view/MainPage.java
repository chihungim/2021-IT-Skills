...
public class MainPage extends BasePage {
	...
	public MainPage() {
		...
		popularP.setBorder(new TitledBorder(new LineBorder(Color.black, 2), "¿Œ±‚ ªÛ«∞", TitledBorder.LEFT,
				TitledBorder.TOP, new Font("∏º¿∫ ∞ÌµÒ", Font.BOLD, 20)));

		eventP.setBorder(new TitledBorder(new LineBorder(Color.black, 2), "¿Ã¥ﬁ¿« 1+1 «‡ªÁ ªÛ«∞", TitledBorder.LEFT,
				TitledBorder.TOP, new Font("∏º¿∫ ∞ÌµÒ", Font.BOLD, 20)));

		Tools.setEmpty(this, 20, 20, 20, 20);
	...
	void popInit() {
		...
		try {
			...
			while (rs.next()) {
				...
				bord.add(img = Tools
						.size(new JLabel(Tools.img("./datafile/image/" + rs.getString(2) + ".jpg", 80, 100)), 80, 100));
				Tools.setLine(img);
				bord.add(new JLabel(rs.getString(2), 0), "South");
				bord.setOpaque(false);
				Tools.setEmpty(bord, 30, 30, 30, 30);
	...
	void eventInit() {
		...
		try {
			...
			while (rs.next()) {
				JPanel bord = new JPanel(new BorderLayout());
				JLabel img;
				bord.add(img = Tools.size(new JLabel(Tools.img("./datafile/image/" + rs.getString(1) + ".jpg", 80, 80)),
						80, 80));
				Tools.setLine(img);
			...
			for (int i = 0; i < events.length; i++) {
				if (events[i].getComponentCount() < 5) {
					for (int j = events[i].getComponentCount(); j < 5; j++) {
						JPanel bord = Tools.size(new JPanel(new BorderLayout()), 100, 100);
						events[i].add(bord);
						bord.setOpaque(false);
					}
				}
