...
public class DeliveryPage extends BasePage {
	...
	public DeliveryPage() {
		...
		scr_p.add(Tools.lbl(sname + "의 물품배송관리", 0, 25), "North");
...
//				9 8 4 10 11 13 12 6 3
				c.add(new ProductItem(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getInt(4), rs.getInt(5),
						rs.getInt(7)));
			}
	...
	class ProductItem extends JPanel {
		...
		public ProductItem(int puno, String name, String user, int stock, int price, int receive) {
			...
			s.add(delivery = Tools.btn("배달하기", e -> {
				System.out.println(puno);
				Tools.execute("insert into receive values(0, '" + puno + "', now())");
				Tools.iMsg("배송을 시작합니다.");
