...
public class Search extends BaseFrame {
	...
	public Search() {
		...
		c_c.add(lbl("현재 예매 가능 공연", JLabel.RIGHT, 13), "North");
		...
		setBorder(cw, new EmptyBorder(50, 0, 50, 0));
	...
	void search() {
		try {
			var rs = stmt.executeQuery("select * from perform where p_date >= '2021-10-06' and pf_no like '%"
					+ Search.this.types + "%' and p_name like '%" + txt.getText()
	...
	class ImagePanel extends JPanel {
		...
		public ImagePanel() {
			...
			addRow(m,
					"select p_no, p_date, p_name, format(p_price, '#,##0') from perform where p_date >= '2021-10-06' and pf_no like '%"
							+ Search.this.types + "%' and p_name like '%" + txt.getText()

							+ "%' order by p_date, p_price desc");
			try {
				var rs = stmt.executeQuery("select * from perform where p_date >= '2021-10-06' and pf_no like '%"
						+ Search.this.types + "%' and p_name like '%" + txt.getText()
						+ "%' group by p_name order by p_date, p_price desc");
				while (rs.next()) {
					...
					Search.this.setBorder(img, new LineBorder(Color.BLACK));
			...
			if (lbls.size() % 4 != 0) {
				for (int i = lbls.size() % 4; i < 4; i++) {
					...
					lbls.add(l);
					Search.this.setBorder(l, new LineBorder(Color.BLACK));
			...
			Search.this.setBorder(this, new EmptyBorder(20, 20, 20, 20));
		}
	}
