...
public class MyPage extends BaseFrame {
	...
	DefaultTableModel m = model("tno,pno,날짜,공연명,좌석,금액".split(","));
	...
	public MyPage() {
		...
		System.out.println("select t_no, p_date, p_name, t_seat, t_discount, p_price from perform p, ticket t where p.p_no=t.p_no and t.u_no="+uno+" order by p_date");
		addRow(m, "select t_no, p.p_no, p_date, p_name, t_seat, t_discount, p_price from perform p, ticket t where p.p_no=t.p_no and t.u_no="+uno+" order by p_date");
		...
		for (int i = 0; i < item.length; i++) {
			...
			item[i].addActionListener(a->{
				...
				if (a.getActionCommand().equals("취소")) {
					iMsg("취소되었습니다.");
					DBManager.execute("delete from ticket where t_no ='"+t.getValueAt(r, 0)+"'");
					addRow(m, "select t_no, p.p_no, p_date, p_name, t_seat, t_discount, p_price from perform p, ticket t where p.p_no=t.p_no and t.u_no="+uno+" order by p_date");
					repaint();
					revalidate();
				} else {
					...
					try {
						var rs = DBManager.rs("select * from perform where p_no="+t.getValueAt(t.getSelectedRow(), 1));
						if (rs.next()) {
	...
	void addRow(DefaultTableModel m, String sql) {
		...
		try {
			...
			while (rs.next()) {
				...
				m.addRow(new Object[] { rs.getString(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5), new DecimalFormat("#,##0").format(sum) });
			}
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
