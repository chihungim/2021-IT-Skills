...
public class Map extends BaseFrame {
	...
	JLabel stations[] = new JLabel[276];
	int dep, arrv;
	...
	void data() {
		...
		adjDim = new int[xy.length][xy.length];
	...
	void ui() {
		mapImg = new JLabel(new ImageIcon(img));
		add(scr = new JScrollPane(mapImg));

		mapImg.setLayout(null);

		for (int i = 1; i <= 275; i++) {
			stations[i] = new JLabel();
			stations[i].setName(i + "");
			stations[i].setBounds(xy[i][0] - 20, xy[i][1] - 10, 25, 25);
			stations[i].setBorder(new LineBorder(Color.BLACK));
			mapImg.add(stations[i]);
			stations[i].addMouseListener(new MouseAdapter() {
				@Override
	...
	void dijkstra() {
		pathDim = new int[5][xy.length]; // 거리, 확정, 이전지점, Cost
		for (int i = 1; i < xy.length; i++) {
	...
	void shortestPath(int arrv) {
		cpath = new ArrayList<Integer>();
		cpath.add(arrv);
		while (pathDim[3][arrv] != -1) {
			cpath.add(pathDim[3][arrv]);
			arrv = pathDim[3][arrv];
		}
			...
			public void paint(Graphics g) {
				try {
					...
					for (int i = 0; i < cpath.size(); i++) {
						xx[i] = xy[cpath.get(i)][0];
						yy[i] = xy[cpath.get(i)][1];
					}
