...
public class BaseFrame extends JFrame {
	...
	static int dijDim[][] = new int[275+1][275+1];
	static int metroTimeDim[][][] = new int[30+1][100][100];
	int INF = 10000000;
	ResultSet rs;
	...
	void dataInit() {
		for (int i = 1; i <= 30; i++) {
			...
			try {
				var rs = DBManager.rs("select * from route where metro= "+i);
				while (rs.next()) {
					l.add(rs.getInt(2));
				}
		...
		try {
			var rs= DBManager.rs("select * from metro");
			while (rs.next()) {
		...
		try {
			var rs = DBManager.rs("select * from station");
			while (rs.next()) {
		...
		for (int i = 1; i <= 30; i++) {
			...
			try {
				var rs= DBManager.rs("select r.station, s.name from route r, station s, metro m where s.serial = r.station and m.serial = r.metro and r.metro=  "+i);
				while (rs.next()) {
		...
		try {
			var rs= DBManager.rs("select * from path");
			cost = new int[584][3];
			int i = 0;
			while (rs.next()) {
		...
		try {
			var rs = DBManager.rs("select p.departure, p.arrive, sub1.name from `path` p, (select distinct r.station st, left(m.name,1) ho, m.name from metro m, route r where r.metro = m.serial) sub1, (select distinct r.station st, left(m.name, 1) ho from metro m, route r where r.metro = m.serial) sub2 where (sub1.st<>sub2.st and sub1.ho=sub2.ho) and (p.departure = sub1.st and p.arrive = sub2.st)");
			while(rs.next()) {
	...
	void dataInit2() throws SQLException {
		...
		//전체 노선별 정류장정보 2차원 배열 
		//metroStInfos[노선의 정류장 ArrayList(노선내 정류장idx <-> 정류장 명)][시작 출차시간][끝 출차 시간][배차 간격]
		for(int i=1; i<=30; i++) {
		...
		for (int i = 1; i < 276; i++) {
			for (int j = i+1; j < 276; j++) {
				dijDim[i][j] = dijDim[j][i] = INF;
			}
		}
		
		rs = DBManager.rs("SELECT * FROM metro.path");
		while (rs.next()) {
			dijDim[rs.getInt(2)][rs.getInt(3)] = rs.getInt(4)*5;
		}
		...
		for(int i=1; i<=30; i++) {
			for(int j=1; j<100; j++) {
				...
				if(j==1) metroTimeDim[i][j][0] = sTime;
				else {
					int from = stNames.indexOf(thisMetro.get(j-1)); 
					int to = stNames.indexOf(thisMetro.get(j));
					int cost = dijDim[from][to];
					metroTimeDim[i][j][0] = metroTimeDim[i][j-1][0] + cost;
				}
	...
	static ImageIcon img(String path, int w, int h) {
		return new ImageIcon(Toolkit.getDefaultToolkit().getImage("./지급자료/images/"+path).getScaledInstance(w, h, Image.SCALE_SMOOTH));
	}
