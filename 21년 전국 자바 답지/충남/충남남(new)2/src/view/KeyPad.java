		...
		n.add(btn("확인", a -> {
			try {
				var rs = stmt.executeQuery(
						"select * from purchase where user=" + uno + " and serial='" + txt.getText() + "'");
				if (rs.next()) {
					new Ticket(rs.getInt(3), rs.getInt(4), LocalDate.parse(rs.getString(7)),
							LocalTime.parse(rs.getString(6)), rs.getDouble(8), rs.getInt(5))
