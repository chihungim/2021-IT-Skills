...
public class PurchaseList extends BaseFrame {
	...
	public PurchaseList() {
		...
		n.add(lblDate = lbl(format.format(date), 0, 13, Color.black, Font.PLAIN));
		n.add(btn("▶", a -> {
			setMonth(1);
		}));
	...
	void setCal() {
		...
		for (int i = 0; i < days.length; i++) {
			...
			try {
				var rs = stmt
						.executeQuery("SELECT s1.name, s2.name FROM purchase pu, station s1, station s2 where user = "
								+ BaseFrame.uno + " and date= '" + l.toString()
								+ "' and s1.serial=pu.departure and s2.serial=pu.arrive order by date, pu.serial");
				String station = "<html>";
				while (rs.next()) {
	...
	class ChartPanel extends JPanel {
		...
		public ChartPanel(String info, Color col, int d) {
			...
			n.add(lbl(String.format("%02d", d), 0, 13, col, Font.PLAIN));
			c.add(lbl(info, 0, 13, Color.black, Font.PLAIN));
