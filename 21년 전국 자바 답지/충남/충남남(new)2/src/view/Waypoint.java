...
public class Waypoint extends JDialog {
	...
	public Waypoint(Reserve r) {
		...
		for (int i = 0; i < r.cpath.size(); i++) {
			if (i == 0)
				way += "<b>" + BaseFrame.stations.get(r.cpath.get(i)) + " - Ãâ¹ß </b><br>";
			else if (i == r.cpath.size() - 1)
				way += "<b>" + BaseFrame.stations.get(r.cpath.get(i)) + "-µµÂø <br>";
			else
		...
		for (int i = 0; i < r.ho.size(); i++) {
			...
			if (transIdx != tmp) {
				if (ways.get(i - 1).matches(".*Ãâ¹ß.*"))
					minus = 10;
				ways.add(i, "<b>" + ways.get(i - 1).substring(0, ways.get(i - 1).length() - minus) + " - ÇÏÂ÷</b>");
				ways.add(i + 1, "<b>" + ways.get(i - 1).substring(0, ways.get(i - 1).length() - minus) + " - È¯½Â"
						+ BaseFrame.lineName.get(r.cpath.get(i) + "," + r.cpath.get(i + 1)) + "(À¸)·Î È¯½Â</b>");
				transIdx = tmp;
			}
		}
