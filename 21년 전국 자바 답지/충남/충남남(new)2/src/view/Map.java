...
public class Map extends BaseFrame {
	...
	JPopupMenu menu[] = new JPopupMenu[276];
	JLabel lbl[] = new JLabel[276];
	...
	public Map() {
		super("지도", 1600, 900);
		JLabel map = new JLabel(getImg("./지급자료/images/metro.png")) {
			@Override
			protected void paintComponent(Graphics g) {
				...
				if (beeLine != null) {
					...
					var xx = new int[beeLine.size()];
					var yy = new int[beeLine.size()];
					g2.setStroke(new BasicStroke(4f));
					for (int i = 0; i < beeLine.size(); i++) {
						xx[i] = n[beeLine.get(i)][0];
						yy[i] = n[beeLine.get(i)][1];
					}

					g2.drawPolyline(xx, yy, beeLine.size());
				}
		...
		add(btn("확인", a -> {
			new Reserve(beeLine).addWindowListener(new Before(Map.this));
		}), "South");
		...
		for (int i = 1; i < n.length; i++) {
			lbl[i] = new JLabel();
			lbl[i].setBounds(n[i][0] - 10, n[i][1] - 10, 20, 20);
			map.add(lbl[i]);
			lbl[i].setComponentPopupMenu(menu[i] = new JPopupMenu());
			for (var bcap : "출발으로 설정,도착으로 설정,해당 역 정보 보기".split(",")) {
				var item = new JMenuItem(bcap);
				item.setName(i + "");
				item.addActionListener(a -> {
					var me = (JMenuItem) a.getSource();
					var serial = rei(me.getName());
					switch (a.getActionCommand()) {
					case "출발으로 설정":
						dep = serial;
						iMsg("출발역을 " + station[serial] + "으로 설정하셨습니다.");
						if (arrv != 0) {
							dijkstra();
					...
					case "도착으로 설정":
						arrv = serial;
						iMsg("도착역을 " + station[serial] + "으로 설정하셨습니다.");
						dijkstra();
						beeLine = getBeepath(arrv);
						repaint();
						break;
					default:
						new Station(serial).addWindowListener(new Before(Map.this));
					}
				});
				menu[i].add(item);
			}
