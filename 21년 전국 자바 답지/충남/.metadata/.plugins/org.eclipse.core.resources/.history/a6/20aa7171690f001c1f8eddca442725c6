...
//		changeL();
//		settime();
//		int bef = atime.getHour(), after = 0;
//		times.delete(0, times.length());
//		
//		int k =0;
//		while(!atime.isAfter(end)) {
//			times.append(tformat(atime, "HH:mm:ss")+"<br>");
//			
//			atime = atime.plusSeconds(inter);
//			after = atime.getHour();
//			if(after > bef) {
...
//			k++;
//		}
//		time = times.toString().split(",");
//		
//		ccc.removeAll();
//		
//		for (int i = 0; i < time.length; i++) {
//			int hour = LocalTime.parse(time[i].substring(0, 8)).getHour();
//			JPanel temp = new JPanel(new FlowLayout());
//			temp.add(new TimeLine(time[i],hour));
//			ccc.add(temp);
//		}
//		ccc.repaint();
//		ccc.revalidate();
...
//	private void changeL() {
//		settime();
//		sta = line.get(stationidx);
//		cur.setText("←" + stations.get(line.get(stationidx))+"→");
//		
//		if(stationidx - 1 == -1 ) {
//			prev.setText("기점");
//		}else {
//			prev.setText(stations.get(line.get(stationidx - 1)));
//		}
//		
//		if(stationidx + 1 == line.size() ) {
//			next.setText("종점");
//		}else {
//			next.setText(stations.get(line.get(stationidx + 1)));
//		}
//	}

//	void settime() {
//		try {
//			ResultSet rs = DBManager.rs("select `start`, `end`, `interval` from metro where name like '%"+mname+"%'");
//			if(rs.next()) {
//				start = LocalTime.parse(rs.getString(1));
//				end = LocalTime.parse(rs.getString(2));
//				inter = LocalTime.parse(rs.getString(3)).toSecondOfDay();
//			}
//			rs.close();
//		} catch (SQLException e) {
//			e.printStackTrace();
//		}
//		atime = start;
//		for (int i = 1; i <= stationidx; i++) {
//			atime = atime.plusSeconds(cost2[line.get(i-1)][line.get(i)] * 5);
//		}
//	}

//	private void setData() {
//		try {
//			mname = combo.getSelectedItem().toString();
//			mserial = metro.get(mname);
//			line = route.get(mserial);
//			stationidx = route.get(mserial).indexOf(sta);
//		} catch (Exception e) {
//			return;
//		}
//	}
...
//		
//		try {
//			ResultSet rs = DBManager.rs("select m.name from route r, metro m where r.metro = m.serial and r.station = "+sta);
//			while(rs.next()) {
