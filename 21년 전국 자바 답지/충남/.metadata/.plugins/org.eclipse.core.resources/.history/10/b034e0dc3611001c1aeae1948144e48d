...
public class EditProfilePage extends BasePage {
	...
	public EditProfilePage() {
		...
		n.add(BasePage.lbl("회원 정보 수정", 0, 25));

		for (int i = 0; i < cap.length; i++) {
			var tmp = new JPanel(new FlowLayout(1));
			if (i == 3)
				tmp.add(BasePage.sz(combo, 150, 25));
			else
				tmp.add(BasePage.sz(txt[i], 150, 25));
			tmp.setOpaque(false);
			c_c.add(tmp);
		}
		...
		try {
			var rs = BasePage.rs("select * from user where u_No=" + uno);
			if (rs.next()) {
				for (int i = 1; i < 4; i++) {
		...
		c_s.add(BasePage.sz(BasePage.btn("수정", a -> {
			...
			execute("update user set u_Pattern = '" + txt[1].getText() + "', u_Name ='" + txt[2].getText()
					+ "', u_Addr = '" + getKey(combo.getSelectedItem().toString()) + "' where u_No=" + uno);
			iMsg("회원정보가 수정되었습니다.");
			mf.swapPage(new MyPage());
		}), 60, 25));

		setBorder(c, new EmptyBorder(30, 0, 30, 0));
		setBorder(this, new EmptyBorder(30, 0, 30, 0));
	...
	void addItem() {
		...
		combo.setSelectedItem(hashPoint.get(BasePage.getOne("SELECT u_Addr from user where u_No=" + uno)));
	}
}
