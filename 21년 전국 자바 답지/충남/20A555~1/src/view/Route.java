...
public class Route extends BaseFrame {
	...
	void setData() {
		...
		try {
			ResultSet rs = DBManager.rs("select r.metro, r.station, s.name, m.name from route r, station s, metro m where s.serial = r.station and m.serial = r.metro and r.metro = "+(combo.getSelectedIndex() + 1));
			while(rs.next()) {
			...
			for (int i = 0; i < x.length; i++) {
				...
				jl.addMouseListener(new MouseAdapter() {
					...
					public void mouseClicked(MouseEvent e) {
						...
						int serial = rei(DBManager.getOne("Select serial from station where name = '"+j.getText()+"'"));
						try {
							hash.clear();
							System.out.println(serial);
							ResultSet rs = DBManager.rs("select * from path where departure = "+serial+" or arrive = "+serial+" group by cost");
							while(rs.next()) {
						...
						try {
							ResultSet rs = DBManager.rs("select r.metro, r.station, s.name, m.name from route r, station s, metro m where s.serial = r.station and m.serial = r.metro and r.station = '"+serial+"'");
