...
public class TimeTable extends BaseFrame {
	...
	void setCenter() {
		...
		c.removeAll();
		try {
			ResultSet rs = stmt.rs("Select r.serial, r.station, s.name, r.metro from route r, station s where r.station = s.serial and metro = "+(mcombo.getSelectedIndex() + 1));
			int i = 1;
			...
			for (int j = 0; j < pa.size(); j++) {
				pa.get(j).addMouseListener(new MouseAdapter() {
					...
					public void mouseClicked(MouseEvent a) {
						...
						try {
							ResultSet rs = stmt.rs("Select r.metro, r.station, s.name, m.name from route r, station s, metro m where s.serial = r.station and m.serial = r.metro and s.name like '%"+map[mcombo.getSelectedIndex() + 1].get(p.serial)+"%' and r.metro <> "+(mcombo.getSelectedIndex() + 1));
							while(rs.next()) {
	...
	private void setWest() {
		...
		start = LocalTime.parse(stmt.getOne("select start from metro where serial = "+(mcombo.getSelectedIndex() + 1)));
		end = LocalTime.parse(stmt.getOne("select end from metro where serial = "+(mcombo.getSelectedIndex() + 1)));
		inter = LocalTime.parse(stmt.getOne("select `interval` from metro where serial = "+(mcombo.getSelectedIndex() + 1)));
