...
public class MyPage extends BaseFrame {
	...
	public MyPage() {
		...
		for (int i = 0; i < bcap.length; i++) {
			w.add(btn(bcap[i], a->{
				if (a.getActionCommand().contentEquals(bcap[0])) {
					...
					if (!stmt.getOne("select * from user where id = '"+txt[0].getText()+"'").isEmpty()) {
					...
					try {
						...
						iMsg("회원정보가 수정되었습니다!");
						stmt.execute("update user set pw = '"+txt[1].getText()+"', name ='"+txt[2].getText()+"', birth = '"+date+"', phone ='"+phone+"' where serial ='"+uno+"'");
		...
		try {
			var rs = stmt.rs("select * from user where serial="+uno);
			if (rs.next()) {
				for (int i = 0; i < 3; i++) {
