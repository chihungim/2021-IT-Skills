...
public class PurchaseList extends BaseFrame {
	...
	void setCal() {
		...
		for (int i = 0; i < days.length; i++) {
			...
			try {
				var rs = stmt.rs("SELECT s1.name, s2.name FROM purchase pu, station s1, station s2 where user = "+uno+" and date= '"+l.toString()+"' and s1.serial=pu.departure and s2.serial=pu.arrive order by date, pu.serial");
				String station = "<html>";
				while (rs.next()) {
