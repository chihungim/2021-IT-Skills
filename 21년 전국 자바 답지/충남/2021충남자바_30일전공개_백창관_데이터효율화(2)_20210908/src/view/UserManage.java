...
public class UserManage extends BaseFrame {
		...
		s.add(btn("저장", a->{
			if(chk) {
				...
				for (var k : hash.keySet()) {
					stmt.execute("update user set pw = '"+hash.get(k)[0]+"', name = '"+hash.get(k)[1]+"', birth = '"+hash.get(k)[2]+"', phone = '"+hash.get(k)[3]+"' where serial = "+k);
				}
			} else {
				eMsg("변경사항이 없습니다.");
			}
	...
	void addRow() {
		m.setRowCount(0);
		try {
			var rs= stmt.rs("select * from user where name like '%"+txt.getText()+"%'");
			while (rs.next()) {
