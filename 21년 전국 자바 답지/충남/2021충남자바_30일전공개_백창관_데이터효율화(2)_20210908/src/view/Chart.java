...
public class Chart extends BaseFrame {
	...
	class ChartPanel extends JPanel {
		...
		public ChartPanel() {
			...
			try {
				var rs = stmt.rs("select u.serial, u.name, count(*) from user u, purchase p where p.`user` = u.serial group by u.`name` order by count(*) desc limit 0,8");
				while(rs.next()) {
	...
	class TimePanel extends JPanel {
		...
		private void evnt() {
			...
			try {
				var rs = stmt.rs("select hour(`time`), sum(if(weekday(`date`) = 6, 1, 0)) as '일', sum(if(weekday(`date`) = 0, 1, 0)) as '월', sum(if(weekday(`date`) = 1, 1, 0)) as '화', sum(if(weekday(`date`) = 2, 1, 0)) as '수', sum(if(weekday(`date`) = 3, 1, 0)) as '목',sum(if(weekday(`date`) = 4, 1, 0)) as '금', sum(if(weekday(`date`) = 5, 1, 0)) as '토' from purchase where `date` >= now() and `date` <= DATE_ADD(now(), INTERVAL "+combo.getSelectedItem().toString().replaceAll("[^0-9]", "")+" DAY) group by `time` order by `time` desc");
				while(rs.next()) {
					for (int i = 0; i < 7; i++) {
